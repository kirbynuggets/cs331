...
// Cart steps
const steps = ['Shopping Bag', 'Shipping Information', 'Payment', 'Confirmation'];
      ...
      <Container maxWidth="xl" sx={{ py: 6 }}>
        <CartEmptyContainer>
          ...
          <Typography variant="body1" color="text.secondary" gutterBottom sx={{ maxWidth: 500, mb: 4 }}>
            Looks like you haven't added anything to your cart yet. Browse our collection to find stylish items for your wardrobe.
          </Typography>
          <Button
          ...
          <Grid container spacing={4}>
            ...
            <Grid item xs={12} md={8}>
              <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
                <Typography variant="h5">Shopping Bag ({items.length} {items.length === 1 ? 'item' : 'items'})</Typography>
                <Button
                  variant="outlined"
                  color="error"
                  size="small"
              ...
              {items.map((item) => (
                <Fade key={item.id} in={true} timeout={500}>
                  <CartItemCard>
                    <CartItemImage component="img" image={item.image_url} alt={item.productDisplayName} />
                    <Box sx={{ display: 'flex', flexDirection: 'column', flexGrow: 1 }}>
                      <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                        <Box>
                          <Typography variant="subtitle1" sx={{ fontWeight: 600 }}>{item.productDisplayName}</Typography>
                          <Typography variant="body2" color="text.secondary">Size: {item.size}</Typography>
                          <Typography variant="body2" color="text.secondary">Color: {item.color || 'Default'}</Typography>
        ...
        return (
          <Box sx={{ 
            ...
            <Typography variant="body1" color="text.secondary" paragraph sx={{ maxWidth: 600 }}>
              ...
              You will receive an email confirmation with the order details.
            </Typography>
