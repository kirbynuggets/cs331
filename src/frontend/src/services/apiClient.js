...
const apiService = {
  ...
  getRandomProducts: (limit = 8, gender = null) =>
    apiClient.get("/random-products", {
      params: { limit, gender },
    }),
  ...
  addToCart: (productId, quantity, price, size, color) =>
    apiClient.post("/cart/add", { productId, quantity, price, size, color }),
  ...
  saveShippingInfo: (shippingInfo) =>
    apiClient.post("/cart/shipping", shippingInfo),
  ...
  getOrderPaymentStatus: (orderId) =>
    apiClient.get(`/orders/${orderId}/payment`),
  ...
  updateUserAddress: (id, addressData) =>
    apiClient.put(`/user/addresses/${id}`, addressData),
