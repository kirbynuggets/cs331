...
def get_item(item_id: str):
    with engine.connect() as connection:
        result = connection.execute(
            text("SELECT * FROM clothing_items WHERE id = :id"),
            {"id": item_id}
        )
...
async def product_page(item_id: str):
    ...
    category_mapping = {
        ...
        'Bottomwear': {
            'compatible_categories': ['Topwear', 'Footwear', 'Belts']
        },
        'Footwear': {
            'compatible_categories': ['Topwear', 'Bottomwear', 'Socks']
        }
    }
    
    compatible_items = get_compatible_items(product, category_mapping.get(
        product['subCategory'], {'compatible_categories': []}), limit=5)
